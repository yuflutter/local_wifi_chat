...
class MessageListModel extends AbstractModel {
  ...
  MessageListModel({super.errorHandler});
  ...
  Future<void> fetchTop() async {
    _refreshTimer?.cancel();
    await _fetch(_FetchMode.top, noSetError: true);
    _refreshTimer = Timer.periodic(di<AppConfig>().refreshListsEvery, (_) async {
      try {
        await fetchNewer();
      } catch (e, s) {
        setError(e, s);
  ...
  Future<void> _fetch(_FetchMode fetchMode, {bool noSetError = false}) async {
    ...
    try {
      switch (fetchMode) {
        ...
        case _FetchMode.older:
          final bath = await repository.fetch(olderThan: list.all.lastOrNull, limit: defaultLimit);
          if (bath.all.isNotEmpty) {
            log.info(null, "Batch of older loaded: ${bath.all.length} items");
          }
          list = MessageList(all: list.all + bath.all, isOlderAvailable: bath.isOlderAvailable);

        case _FetchMode.newer:
          final bath = await repository.fetch(newerThan: list.all.firstOrNull, limit: 0, noLog: true);
          if (bath.all.isNotEmpty) {
            log.info(null, "Batch of newer loaded: ${bath.all.length} items");
            _existsUnread = true;
          }
          list = MessageList(all: bath.all + list.all, isOlderAvailable: list.isOlderAvailable);
      }
      //
    } catch (e) {
      if (!noSetError) setError(e);
      rethrow;
    } finally {
      notifyListeners();
    }
